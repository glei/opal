 ...
 * $Revision: 26464 $
 * $Author: rjongbloed $
 * $Date: 2011-09-24 09:21:46 +0900 (土, 24 9 2011) $
 */
...
static unsigned   MyMaxFrameRate = 60;//60->15;                      // Maximum frame rate (per second)
...
static struct PluginCodec_Option const MaxBR =
{
  ...
  0,                                  // H.245 generic capability code and bit mask
  "0",                                // Minimum value
  "240000"                            // Maximum value 240000->240
};
...
class MyEncoder : public PluginCodec<MY_CODEC>
{
  ...
  public:
    MyEncoder(const PluginCodec_Definition * defn)
      ...
      , m_width(MY_VIDEO_WIDTH)
      , m_height(MY_VIDEO_HEIGHT)
      , m_frameRate(1)
      , m_bitRate(512000)
      ...
      , m_keyFramePeriod(0)
    {
	// PTRACE(1, MY_CODEC_LOG, "99999h264!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!");
    }
...
// PTRACE(1, MY_CODEC_LOG, "h264!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!1");
      // ABR with bit rate tolerance = 1 is CBR...
      // if (FFMPEGLibraryInstance.Load() && m_encoder.Load(this))
      //   return true;
		avcodec_init();
// PTRACE(1, MY_CODEC_LOG, "h264!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!2");		
		// H264Encoder *Favcodec_h264_decoder=&m_encoder;
		register_avcodec(&h264_decoder);
// PTRACE(1, MY_CODEC_LOG, "h264!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!3");		
		// x264_encoder_open(&m_context);
		m_encoder.Load(this);
		// PTRACE(1, MY_CODEC_LOG, "h264!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!3");
		return true;
...
class MyDecoder : public PluginCodec<MY_CODEC>
{
  ...
  public:
    ...
    virtual bool Construct()
    {
	  ...
	  av_log_set_level(-8);//2012.04.13 kou add
      if ((m_codec = avcodec_find_decoder(CODEC_ID_H264)) == NULL)
        return false;

      // if ((m_context = FFMPEGLibraryInstance.AvcodecAllocContext()) == NULL)
	  if ((m_context = avcodec_alloc_context()) == NULL)
        return false;
      ...
      if ((m_picture = avcodec_alloc_frame()) == NULL)
        return false;
                             ...
                             unsigned & flags)
    {
      ...
      //                                                             bytesToDecode);
		int bytesDecoded = avcodec_decode_video(m_context,
												m_picture,
												&gotPicture,
												m_fullFrame.GetFramePtr(),
												bytesToDecode);
...
extern "C"
{
  ...
  const struct PluginCodec_Definition * PLUGIN_CODEC_GET_CODEC_FN(unsigned * count, unsigned version)
  {
