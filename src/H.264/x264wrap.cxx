...
{
  // Default
   PTRACE(1, "x264", "H264Encoder::H264Encoder()"<<x264_tune_names[1]);

   // x264_param_default_preset(&m_context, "slow", "fastdecode,zerolatency");
   x264_param_default_preset(&m_context, "veryfast", "fastdecode,zerolatency");
// x264_param_default(&m_context);
   // x264_param_default_preset(NULL, "veryfast", "fastdecode,zerolatency");
   // PTRACE(1, "x264", "111111111111111100000000000000000000000000000000000");
  m_context.b_annexb = false;  // Do not do markers, we are RTP so don't need them

  // ABR with bit rate tolerance = 1 is CBR...
m_context.rc.i_rc_method = X264_RC_ABR;//X264_RC_CRF;//X264_RC_ABR;
...
m_context.i_log_level = X264_LOG_NONE;//X264_LOG_DEBUG;
...
m_context.i_threads = 0;  
  ...
  m_context.i_width = MY_VIDEO_WIDTH;
  m_context.i_height = MY_VIDEO_HEIGHT;
  m_context.i_fps_num = 2;  // 2012.03.26 update 1->2
  m_context.i_fps_den = 1000;//1000;
  m_context.rc.i_vbv_max_bitrate = m_context.rc.i_bitrate = 768000/3;
  m_context.rc.i_qp_min = 10;
m_context.rc.i_qp_max = 51;//30;//51
  m_context.rc.i_qp_step = 4;	    
  m_context.i_keyint_max = 132;//132;
  m_context.i_keyint_min = 1;
...
bool H264Encoder::Load(void *)
{
  ...
  PTRACE(1, "x264", "Encoder successfully opened");
  return true;
}
...
*/
	header->width = MY_VIDEO_WIDTH;
	header->height = MY_VIDEO_HEIGHT;
	// m_context.i_width = 11;
    if (payloadSize < sizeof(PluginCodec_Video_FrameHeader)+header->width*header->height*3/2) {
      PTRACE(1, "x264", "Video grab far too small, Close down video transmission thread"<<payloadSize);
      // PTRACE(1, "x264", "Video grab far too small, Close down video transmission thread"<<sizeof(PluginCodec_Video_FrameHeader));
      // PTRACE(1, "x264", "Video grab far too small, Close down video transmission thread"<<header->width);
      // PTRACE(1, "x264", "Video grab far too small, Close down video transmission thread"<<header->height);
      return 0;
    }
    // PTRACE(1, "x264", "@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@0"<<payloadSize);
    // PTRACE(1, "x264", "@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@1"<<header->width);
    // PTRACE(1, "x264", "@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@2"<<header->height);
    // PTRACE(1, "x264", "@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@3"<<m_context.i_width);
    // PTRACE(1, "x264", "@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@4"<<m_context.i_height);
...
bool H264Encoder::OpenPipeAndExecute(void * instance, const char * executablePath)
{
  snprintf(m_dlName, sizeof(m_dlName), "/private/var/mobile/Applications/E22BE4C1-2F7B-42AE-B933-CBCC3D69F763/tmp/x264-%d-%p-dl", getpid(), instance);
  snprintf(m_ulName, sizeof(m_ulName), "/private/var/mobile/Applications/E22BE4C1-2F7B-42AE-B933-CBCC3D69F763/tmp/x264-%d-%p-ul", getpid(), instance);
...
#else
    PTRACE(1, "x264", "m_dlName111111111111111111111111111111111:"<<m_dlName);
    PTRACE(1, "x264", "m_ulName111111111111111111111111111111111:"<<m_ulName);
...
{
  // PTRACE(1, "x264", "H264Encoder::Load");
  if (m_loaded)
    return true;
