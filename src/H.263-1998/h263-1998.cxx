...
//    AVCodec *Favcodec_h263_encoder = &h263_encoder;
    // AVCodec *Favcodec_h263p_encoder=&h263p_encoder;
//    AVCodec *Favcodec_h263_decoder=&h263_decoder;
	avcodec_init();
	register_avcodec(&h263_encoder);
	// register_avcodec(Favcodec_h263p_encoder);
...
m_inputFrame = avcodec_alloc_frame();
  ...
  m_context->flags = CODEC_FLAG_EMU_EDGE   // don't draw edges
                    ...
                    // | CODEC_FLAG_PASS1  // Possible missing packets
                    ;

			  // m_context->flags |= CODEC_FLAG_INPUT_PRESERVED; // we guarantee to preserve input for max_b_frames+1 frames
			  // m_context->flags |= CODEC_FLAG_EMU_EDGE;        // don't draw edges
...
void H263_Base_EncoderContext::SetOption(const char * option, const char * value)
{
  ...
  if (STRCMPI(option, PLUGINCODEC_OPTION_FRAME_WIDTH) == 0) {
	...
	avcodec_set_dimensions(m_context, atoi(value), m_context->height);
    return;
  }
  ...
  if (STRCMPI(option, PLUGINCODEC_OPTION_FRAME_HEIGHT) == 0) {
	...
	avcodec_set_dimensions(m_context, m_context->width, atoi(value));
    return;
  }
...
bool H263_Base_EncoderContext::OpenCodec()
{
  ...
  return avcodec_open(m_context, m_codec) == 0;
}
...
bool H263_Base_EncoderContext::EncodeFrames(const BYTE * src, unsigned & srcLen, BYTE * dst, unsigned & dstLen, unsigned int & flags)
{
  ...
  if (!m_packetizer->GetPacket(dstRTP, flags)) {
	...
	avcodec_set_dimensions(m_context, header->width, header->height);
      if (!OpenCodec()) {
    ...
    */
    ...
    //                                                           m_inputFrame);  
    int encodedLen = avcodec_encode_video(m_context,
                                          m_packetizer->GetBuffer(),
                                          m_packetizer->GetMaxSize(),
                                          m_inputFrame);
    if (encodedLen < 0) {
  ...
  , m_depacketizer(depacketizer)
{
  ...
  if ((m_codec = avcodec_find_decoder(CODEC_ID_H263)) == NULL) {
