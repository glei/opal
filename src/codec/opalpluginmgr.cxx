...
// static int framesFlg = 0;
bool OpalPluginVideoTranscoder::DecodeFrames(const RTP_DataFrame & src, RTP_DataFrameList & dstList)
{
	        // PTRACE(1, "OpalPlugin\tsrc.GetMarker():"<<src.GetMarker());
	// if (framesFlg == 25 && src.GetMarker() == 0)
	// {
	// 	PThread::Sleep(40);
	// 	return true;
	// }else if (framesFlg == 25 && src.GetMarker() == 1){
	// 	framesFlg =0;
	// 	PThread::Sleep(40);
	// 	return true;
	// }
	// 
	// if (src.GetMarker() == 1){
	// 	framesFlg = framesFlg+1;
	// }
		// PTRACE(1, "DecodeFrame5555555");
		// return true;
...
{
	// PTRACE(1, "DecodeFrame1111111111");
  // call the codec function
...
H323Capability *CreateGenericVideoCap(const PluginCodec_Definition * codecDefn,
                                      ...
                                      int /*subType*/) 
{
  PTRACE(4, "OpalPlugin\tCreateGenericVideoCap......................................1");
...
{
	// PTRACE(1, "OpalPlugin\t RegisterCapability is :" << codecDefn);
  ...
  if (isValid.Exists() && !isValid.Call((void *)"h323", sizeof(const char *))) {
    PTRACE(1, "OpalPlugin\tNot adding H.323 capability for plugin codec " << codecDefn->descr << " as this has been specifically disabled");
    return;
  }
  ...
  for (PINDEX i = 0; H323CapabilityMaps[i].pluginCapType >= 0; i++) {
    if (H323CapabilityMaps[i].pluginCapType == capabilityType) {
      ...
      // PTRACE(1, "OpalPlugin\t mediaFormat is :" << mediaFormat);
      ...
      if (H323CapabilityMaps[i].createFunc != NULL)
        ...
        PTRACE(1, "OpalPlugin\tNo capability creation function for " << codecDefn->descr);
      }
      ...
      if (cap != NULL) {
        ...
        PTRACE(1, "OpalPlugin\tNo capability created for " << codecDefn->descr);
      }
      break;
    }
  }
...
extern const struct PluginCodec_Definition * Opal_StaticCodec_##name##_GetCodecs(unsigned *,unsigned); \
}; \
...
#endif
// kou 2012.02.15 del start
// INCLUDE_STATIC_CODEC(FFMPEG_H263P)
// kou 2012.02.15 del end
INCLUDE_STATIC_CODEC(MY_CODEC)
