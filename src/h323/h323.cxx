 ...
 * $Revision: 26530 $
 * $Author: csoutheren $
 * $Date: 2011-10-05 16:23:48 +0900 (水, 05 10 2011) $
 */
...
// 2012.04.02 kou add start
extern "C" {
	int stateCon;
};
// 2012.04.02 kou add end
PBoolean H323Connection::HandleSignalPDU(H323SignalPDU & pdu)
{
  ...
  PBoolean ok;
PTRACE(1, "q931.GetMessageType() \"" << q931.GetMessageType() << '"');
...
{
// PTRACE(1, "H225\tH.245 HandleTunnelPDU.............");
...
// PTRACE(1, "H225\tH.245 5555555555555555555555555555555555");
  }else {
...
// PTRACE(1, "H225\tH.245 111111111111111111111111111111111111111");
...
//cyoudatu
  		// PTRACE(1, "calltype is %02d " << iCallType);
        arq.IncludeOptionalField(H225_AdmissionRequest::e_nonStandardData);
        PString UserInfo;
        UserInfo.sprintf("CallType=%02d", iCallType);
        arq.m_nonStandardData.m_data=UserInfo;
//cyoudatu
...
{
	// PTRACE(1, "H4502\t OnReceivedSignalConnect.........................");
  if (GetPhase() < AlertingPhase) {
  ...
  // PTRACE(1, "H225\tNo H245 33333333333333333333333333333333333333");
...
PBoolean H323Connection::ForwardCall(const PString & forwardParty)
{
  ...
  PTRACE(1, "H4502\t 44444444444444444444444444444444444444444444444");
...
// PTRACE(1, "H245\t WriteControlPDU start.........");
  if (!h245Tunneling) {
    if (controlChannel == NULL) {
...
// PTRACE(1, "H245\t WriteControlPDU end..........");
...
// PTRACE(1, "H245\tWrite PDU fail: 66666666666666666666666666666666666666666");	
  }else {
// PTRACE(1, "H245\tWrite PDU fail: 22222222222222222222222222222222222222222");	
    localTunnelPDU.BuildFacility(*this, true);
...
{
  // PTRACE(1, "OpalCon\t H323Connection::OpenMediaStream VVVVVVVVVVVVVVVVVVVVVVVVVVVVVV................. " );
  // See if already opened
  OpalMediaStreamPtr stream = GetMediaStream(sessionID, isSource);
  if (stream != NULL && stream->IsOpen()) {
    if (stream->GetMediaFormat() == mediaFormat) {
      PTRACE(1, "H323\tOpenMediaStream (already opened) for session " << sessionID << " on " << *this);
      return stream;
    }
...
void H323Connection::SelectDefaultLogicalChannel(const OpalMediaType & mediaType, unsigned sessionID)
{
  ...
  if (otherConnection == NULL) {
    PTRACE(1, "H323\tSelectLogicalChannel(" << sessionID << ") cannot start channel without second connection in call.");
    return;
  }
	// PTRACE(1, "H323\tSelectLogicalChannel(PPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPP");
  if (!ownerCall.OpenSourceMediaStreams(*otherConnection, mediaType, sessionID)) {
    PTRACE(4, "H323\tSelectLogicalChannel(" << sessionID << ") could not start media stream.");
  }
}
                                                                        ...
                                                                        RTP_QOS * rtpqos)
{
  ...
  // PTRACE(1, "H323\tCreateRealTimeLogicalChannel..............................99999999999999991 " << capability);
  ...
  if (!transport.IsCompatibleTransport("ip$127.0.0.1"))
    return NULL;
  // PTRACE(1, "H323\tCreateRealTimeLogicalChannel..............................99999999999999992 " << capability);
  H323RTPSession * session = dynamic_cast<H323RTPSession *>(UseMediaSession(sessionID, mediaType));
  if (PAssertNULL(session) == NULL)
    return NULL;
  // PTRACE(1, "H323\tCreateRealTimeLogicalChannel..............................99999999999999993 " << capability);
  ...
  // PTRACE(1, "H323\tCreateRealTimeLogicalChannel..............................99999999999999994 " << capability);
#if P_QOS
...
{
	  PTRACE(1, "H323\t OnAcceptModeChange WWWWWWWWWWWWWWWWWWWWWWWWWW");
  if (t38ModeChangeCapabilities.IsEmpty())
    return;
...
{
#if OPAL_H460
    // PTRACE(1, "RAS\3333333333333333333333333333");
  return features->SendFeature(code, featureSet);
#else
//    PTRACE(1, "RAS\4444444444444444444444444444");
