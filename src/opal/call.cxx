 ...
 * $Revision: 26542 $
 * $Author: rjongbloed $
 * $Date: 2011-10-06 09:16:06 +0900 (木, 06 10 2011) $
 */
...
{
	 // PTRACE(1, "Call\tOpenSourceMediaStreams YYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYY");
  ...
  if (IsOnHold()) {
    PTRACE(1, "Call\tOpenSourceMediaStreams (call on hold)" << traceText);
    return false;
  }
  ...
  while (EnumerateConnections(otherConnection, PSafeReadWrite, &connection)) {
    ...
    if (sourceFormat.IsValid())
      ...
      if (sourceMediaFormats.IsEmpty()) {
        PTRACE(1, "Call\tOpenSourceMediaStreams failed with no source formats" << traceText);
        return false;
      }
    ...
    if (contentRole != OpalVideoFormat::eNoRole) {
      ...
      for (PINDEX i = 0; i < 2; i++) {
        ...
        if (lists[i]->IsEmpty()) {
          PTRACE(1, "Call\tUnsupported Content Role " << contentRole << traceText);
          return false;
        }
      }
    }
    ...
    if (!SelectMediaFormats(mediaType,
                            ...
                            sourceFormat,
                            sinkFormat)){
	 PTRACE(1, "HHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHH ");
      return false;
}
#if OPAL_VIDEO
    ...
    if (sourceStream == NULL) {
      ...
      if (sourceStream == NULL){
	 PTRACE(1, "HHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHH 1111111111111111111111");
        return false;
	}
    ...
    if (sinkStream != NULL) {
      ...
      if (!sourceStream.SetSafetyMode(PSafeReadOnly)){
		 PTRACE(1, "HHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHH 22222222222222222222");
        return false;
	}
      OpalMediaPatch * patch = sourceStream->GetPatch();
      if (patch == NULL) {
        ...
        if (patch == NULL){
		 PTRACE(1, "HHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHH 33333333333333333333");
          return false;
		}
      }
...
{
	// PTRACE(1, "Call\tSelectMediaFormats could not find compatible " << mediaType << " format:\n"
  ...
  if (OpalTranscoder::SelectFormats(mediaType, srcFormats, dstFormats, allFormats, srcFormat, dstFormat)) {
    PTRACE(2, "Call\tSelected media formats " << srcFormat << " -> " << dstFormat);
    return true;
  }
...
//  fprintf(stderr, "hhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhh:void OpalCall::SetPartyNames()\n");
  ...
  if (connectionA == NULL)
    return;
// fprintf(stderr, "hhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhh:connectionA <> NULL\n");
