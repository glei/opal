 ...
 * $Revision: 26493 $
 * $Author: rjongbloed $
 * $Date: 2011-09-28 16:14:12 +0900 (水, 28 9 2011) $
 */
...
{
  // PTRACE2(1, NULL, "Socket\tOpalTransportAddress::GetIpAndPort."<<*this);
  if (transport == NULL)
    return PFalse;
...
{
  // PTRACE(1, "start ->>>>>>Split " << addr << " into host='" << host << "',dev='" << device << "',service='" << service << "'");
  // skip transport identifier
  PINDEX i = addr.Find('$');
  if (i == P_MAX_INDEX) {
...
{
  PString host, device, service;
 // PTRACE(1, "hhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhh "<<address);
  if (!SplitAddress(address, host, device, service))
    return PFalse;
...
// PTRACE(1, "Opal\tIllegal IP transport port/service: \"" << address << '"');
    if (port == 0) {
                                 ...
                                 PBoolean & reuseAddr)
{
 ...
 // PTRACE2(1, NULL,"Listen\tOpalInternalIPTransport................." << port);
  switch (option) {
...
void OpalTransport::CloseWait()
{
  ...
  if (exitingThread != NULL) {
	...
	// 2012.04.04 kou update start
      PAssert(exitingThread->WaitForTermination(10000*10), "Transport thread did not terminate");
	// 2012.04.04 kou update end
      delete exitingThread;
    }
  }
}
...
{
	  // PTRACE(1, "99999999999999999999999999999999"<<localAddress);
...
OpalTransportAddress OpalTransportUDP::GetLocalAddress(bool allowNAT) const
{
  ...
  // PTRACE(1, "555555555555555555555555555555555555555 "<<thisWritable->localPort);
  }
