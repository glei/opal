 ...
 * $Revision: 26523 $
 * $Author: rjongbloed $
 * $Date: 2011-10-05 14:06:52 +0900 (水, 05 10 2011) $
 */
...
// PString filename = "/private/var/mobile/Applications/179A8162-DAD1-4921-9804-D90FE69125A3/tmp/123.log";
#endif
...
#endif
	 PINDEX calltype = options.Find("@1");
	 if (calltype < P_MAX_INDEX) {
		iCallType = 1;
	 }
	
	 calltype = options.Find("@3");
	 if (calltype < P_MAX_INDEX) {
		iCallType = 3;
	 }
	 
	 if(iCallType != 1 && iCallType != 3)
	{
		iCallType = 1;
	} 
...
{
	// PTRACE(4, "OpalC API\tHandling message " << "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaall"<< " from application");
...
void OpalManager_C::HandleSetGeneral(const OpalMessage & command, OpalMessageBuffer & response)
{
  ...
  OpalMediaTypeList allMediaTypes = OpalMediaType::GetList();
	// PTRACE(4,"tttttttttttttttttttttttttttttttttttt:");
  for (OpalMediaType::AutoStartMode autoStart = OpalMediaType::Receive; autoStart < OpalMediaType::ReceiveTransmit; ++autoStart) {
	...
	// PTRACE(4,"lllllllllllllllllllllllllllllllllllllllllllllllllllllllll:");
    PString autoXxMedia;
  ...
  if (!IsNullString(command.m_param.m_general.m_natRouter)) {
	...
	// PTRACE(4,"hhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhh:");
#ifdef P_STUN
...
#endif
	// PTRACE(4,"kkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkk:");
  ...
  for (PINDEX i = 0; i < options.GetSize(); ++i) {
    ...
    if (mediaName.IsEmpty() || optionName.IsEmpty()) {
      PTRACE(1, "OpalC API\tInvalid syntax for media format option: \"" << optionSpec << '"');
    }
    else {
      ...
      if (OpalMediaType::GetDefinition(mediaType) != NULL) {
        ...
        for (OpalMediaFormatList::iterator it = allCodecs.begin(); it != allCodecs.end(); ++it) {
          if (it->GetMediaType() == mediaType) {
            if (it->SetOptionValue(optionName, optionValue)) {
              ...
              PTRACE(1, "OpalC API\tSet " << mediaType << " media format \"" << *it
                     << "\" option \"" << optionName << "\" to \"" << optionValue << '"');
            }
            else {
              PTRACE(1, "OpalC API\tCould not set " << mediaType
        ...
        if (mediaFormat.IsValid()) {
          if (mediaFormat.SetOptionValue(optionName, optionValue)) {
            ...
            PTRACE(1, "OpalC API\tSet media format \"" << mediaFormat
                   << "\" option \"" << optionName << "\" to \"" << optionValue << '"');
          }
          else {
            PTRACE(1, "OpalC API\tCould not set media format \"" << mediaFormat
                   << "\" option \"" << optionName << "\" to \"" << optionValue << '"');
          }
        }
        else {
          PTRACE(1, "OpalC API\tTried to set option for unknown media format: \"" << mediaName << '"');
        }
      }
    }
  }
...
//   if (is_user ==1)
// {
// 	PTRACE(1, "StartStopListeners is stop");
// 	return;	
// } 
  if (ep == NULL)
    return;
...
//fprintf(stderr, "hhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhh99999999999\n");
...
//	fprintf(stderr, "hhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhh123456\n");
}
...
{
//fprintf(stderr, "hhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhh:SendMessage\n");
  if (m_handle == NULL) {
    response.SetType(OpalIndCommandError);
    response.m_message->m_param.m_commandError = "Uninitialised OPAL context.";
//	fprintf(stderr, "hhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhh:Uninitialised OPAL context.\n");
    return false;
  }

  response.m_message = OpalSendMessage(m_handle, message.m_message);
  if (response.m_message != NULL){
//	fprintf(stderr, "hhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhh:response.m_message != NULL\n");
    return response.GetType() != OpalIndCommandError;
  }
  response.SetType(OpalIndCommandError);
  response.m_message->m_param.m_commandError = "Invalid message.";
//  fprintf(stderr, "hhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhh:Invalid message.\n");
  return false;
}
...
{
//  fprintf(stderr, "hhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhh:SetUpCall Start.\n");
