 ...
 * $Revision: 26581 $
 * $Author: rjongbloed $
 * $Date: 2011-10-12 18:14:39 +0900 (水, 12 10 2011) $
 */
...
//	fprintf(stderr, "hhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhh:!OnIncomingConnection(0, NULL) 111111111\n");
...
void OpalConnection::AutoStartMediaStreams(bool force)
{
  ...
  for (OpalMediaTypeList::iterator iter = mediaTypes.begin(); iter != mediaTypes.end(); ++iter) {
    ...
    if ((GetAutoStart(mediaType)&OpalMediaType::Transmit) != 0 && (force || GetMediaStream(mediaType, true) == NULL)){
		// PTRACE(1, "OpalCon\tAutoStartMediaStreams OOOOOOOOOOOOOOOOOOOOOOOOOO ");
...
{
	// PTRACE(1, "OpalCon\t OpenMediaStream XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX................. " );
  PSafeLockReadWrite safeLock(*this);
  ...
  if (stream != NULL && stream->IsOpen()) {
    if (stream->GetMediaFormat() == mediaFormat) {
      PTRACE(1, "OpalCon\tOpenMediaStream (already opened) for session " << sessionID << " on " << *this);
      return stream;
    }
