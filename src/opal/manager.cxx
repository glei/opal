...
OpalManager::OpalManager()
  ...
  , rtpPayloadSizeMax(1536-20-16-12) // Max safe MTU size (576 bytes as per RFC879) minus IP, UDP an RTP headers
...
//fprintf(stderr, "hhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhh:OpalManager::SetUpCall\n");
  OpalCall * call = CreateCall(userData);
  if (call == NULL){
//	fprintf(stderr, "hhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhh:call == NULL\n");
    return NULL;
  }
...
//fprintf(stderr, "hhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhh:Could not create connection\n");
...
//fprintf(stderr, "hhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhh:OpalManager::MakeConnection\n");
  if (remoteParty.IsEmpty())
    return NULL;
//fprintf(stderr, "hhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhh:remoteParty.IsEmpty\n");
  ...
  if (ep != NULL){
//	fprintf(stderr, "hhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhh:ep <> NULL\n");
...
//fprintf(stderr, "hhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhh:PBoolean OpalManager::OnIncomingConnection \n");
  ...
  if (destination.IsEmpty()) {
    ...
    if (destination.IsEmpty()) {
//	fprintf(stderr, "hhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhh:Cannot complete call, no destination address from connection\n");
...
//fprintf(stderr, "hhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhh:OnRouteConnection(routesTried, connection.GetLocalPartyURL(), destination, call, options, &mergedOptions)\n");
...
//	  fprintf(stderr, "hhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhh:Could not route a=\n");
...
//fprintf(stderr, "PBoolean OpalManager::AddRouteEntry(const PString & spec) %s\n",(const char*)spec);
...
// fprintf(stderr, "m_routeTable.GetSize() %s\n",  (const char *)search);
// fprintf(stderr, "m_routeTable.GetSize() %d\n",m_routeTable.GetSize());
  PString destination;
  while (routeIndex < m_routeTable.GetSize()) {
    RouteEntry & entry = m_routeTable[routeIndex++];
//    fprintf(stderr, "entry.pattern %s\n",(const char *)entry.pattern);
    PINDEX pos;
...
//	   fprintf(stderr, "destination = entry.destination.\n");
        destination = entry.destination;
        break;
      }

      // restart search in table using label.
// fprintf(stderr, "restart search in table using label. %s\n",  (const char *)search);
      search = entry.destination;
      routeIndex = 0;
    }
    else {
      PTRACE(4, "OpalMan\tDid not match regex \"" << entry.regex.GetPattern() << "\" (\"" << entry.pattern << "\")");
//      fprintf(stderr, "OpalMan\tDid not match regex\n");
    }
  }
